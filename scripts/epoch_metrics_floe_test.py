...
from src.datasets.floe import DatasetValidateFloe, DatasetTestFloe
...
if __name__ == '__main__':
    ...
    for epoch in range(e):
        ...
        for batch in tqdm(val_loader):
            ...
            tot_iou += jaccard_index(hist)[0].item()
            tot_acc += per_class_pixel_accuracy(hist)[0].item()
    ...
    with open(f'../results/floe/test_set/{model_name}-mean-acc-epoch.json', 'w') as fp:
        json.dump(acc_dict, fp)

    with open(f'../results/floe/test_set/{model_name}-mean-iou-epoch.json', 'w') as fp:
