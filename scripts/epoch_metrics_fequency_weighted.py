...
import numpy as np
from src.metrics.segmentation import _fast_hist, per_class_pixel_accuracy, jaccard_index, fw_miou, overall_pixel_accuracy
...
if __name__ == '__main__':
    ...
    for epoch in range(N_EPOCHS):
        ...
        acc_dict[epoch + 1] = overall_pixel_accuracy(hist).item()
        iou_dict[epoch + 1] = fw_miou(hist).item()
    ...
    with open(f'../results/val_set_fw/{model_name}-mean-acc-epoch.json', 'w') as fp:
        json.dump(acc_dict, fp)

    with open(f'../results/val_set_fw/{model_name}-mean-iou-epoch.json', 'w') as fp:
