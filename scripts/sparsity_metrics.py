...
def get_args():
    ...
    parser.add_argument('-m', '--model', dest='model', type=str, default='small_unet',
                        help='Model to use.')
    parser.add_argument('-c', '--checkpoint', dest='checkpoint', type=str, default='skinny_small_lbc_unet_sparcity_50',
                        help='Model to use.')
    parser.add_argument('-e', '--epochs', dest='epochs', type=int, default=20,
                        help='Number of checkpoint epochs.')
...
def get_model_and_checkpoint(model_name, sparsity=0.5):
...
if __name__ == '__main__':
    ...
    log.info(f'Evaluating {args.model} with sparsity {sparsity}')
    ...
    for epoch in range(N_EPOCHS):
        net = get_model_and_checkpoint(args.model, sparsity)
    ...
    with open(f'../results/sparsity_tests/{model_name}-mean-acc-epoch.json', 'w') as fp:
        json.dump(acc_dict, fp)

    with open(f'../results/sparsity_tests/{model_name}-mean-iou-epoch.json', 'w') as fp:
        json.dump(iou_dict, fp)

    with open(f'../results/sparsity_tests/{model_name}-mean-loss-epoch.json', 'w') as fp:
