...
class IceWithProposals(Dataset):
    ...
    def resize(self, pil_img, is_prop=False, is_img=False):
    ...
    def process(self, img, mask, prop):
        img = self.resize(img, is_img=True)
...
class BasicDatasetIce(Dataset):
    def __init__(self, imgs_dir, masks_dir, txt_dir, split, scale=1, mask_suffix='', preprocessing=None,
                 augmentation=None, crop=300):
        self.imgs_dir = imgs_dir
        self.masks_dir = masks_dir
        self.txt_dir = txt_dir
        self.split = split
