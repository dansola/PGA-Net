...
def train_net(net, data_dir, device, epochs=20, batch_size=1, lr=0.0001, save_cp=True, img_scale=0.35, img_crop=320,
    ...
    train_set = Ice(os.path.join(data_dir, img_dir), os.path.join(data_dir, 'masks'),
                    os.path.join(data_dir, f'txt_files/cross_val/cv{cv}'), 'train', img_scale, img_crop)
    val_set = Ice(os.path.join(data_dir, 'imgs'), os.path.join(data_dir, 'masks'),
                  os.path.join(data_dir, f'txt_files/cross_val/cv{cv}'), 'val', img_scale, 256)
...
if __name__ == '__main__':
    ...
    try:
        ...
        train_net(net=net, data_dir=args.data_dir, epochs=args.epochs, batch_size=args.batchsize, lr=args.lr,
                  device=device,
                  img_scale=args.scale, img_crop=crop, img_dir=args.imgdir, cv=args.cv)
