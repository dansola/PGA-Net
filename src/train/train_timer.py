...
if __name__ == '__main__':
    ...
    try:
        ...
        if os.path.exists(f'model_profile_{args.device}_v3.json'):
            with open(f'model_profile_{args.device}_v3.json') as f:
                data = json.load(f)
            data[args.model] = run_time
            with open(f'model_profile_{args.device}_v3.json', 'w') as outfile:
